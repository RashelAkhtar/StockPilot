...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT * FROM product_list WHERE user_id = $1 ORDER BY id ASC",
      [userId],
    );
...
router.post("/", validate(productSchema), async (req, res) => {
  try {
    ...
    if (!productName)
      return res.status(400).json({ err: "productName is required" });
    if (!Number.isFinite(normalizedBuyingPrice) || normalizedBuyingPrice < 0) {
      return res
        .status(400)
        .json({ err: "buyingPrice must be a valid number" });
    }
    if (!Number.isInteger(normalizedProductQty) || normalizedProductQty <= 0) {
      return res
        .status(400)
        .json({ err: "productQty must be a whole number greater than 0" });
    }
    ...
    if (existing.rowCount > 0) {
      ...
      return res
        .status(200)
        .json({ message: "Product updated", data: updated.rows[0] });
    }
